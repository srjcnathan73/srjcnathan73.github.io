...
function initClickHandler() {
	$('.pad').click( function(){
		if (active===true) {
			...
			if (!hardMode){
			flash( $(this),1,1900, pad );
	}		else {flash( $(this),1,300, pad );}
			logPlayerSequence(pad);
			document.getElementById('sound1').play().duration = .001;
			}
	});
	handler = true;
}
	...
	if (times > 0) {
		...
		element.stop().animate(	
			{opacity: '2'}, 
			{
				duration: 8,
				complete: function(){
					element.stop().animate({opacity: '.5'}, 2000);
				}
			} 
		);}
		else {element.stop().animate( 
			{opacity: '1'}, 
			{
				duration: 50,
				complete: function(){
...
// generate random numbers (1-4) and push onto generated array 
